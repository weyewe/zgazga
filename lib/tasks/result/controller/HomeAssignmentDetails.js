Ext.define('AM.controller.HomeAssignmentDetails', {
  extend: 'Ext.app.Controller',

  stores: ['HomeAssignmentDetails', 'HomeAssignments'],
  models: ['HomeAssignmentDetail'],

  views: [
    'operation.homeassignmentdetail.List',
    'operation.homeassignmentdetail.Form',
  ...
  init: function() {
    this.control({
      'homeassignmentdetaillist': {
        itemdblclick: this.editObject,
        selectionchange: this.selectionChange ,
				afterrender : this.loadObjectList
      },
      'homeassignmentdetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'homeassignmentdetaillist button[action=addObject]': {
        click: this.addObject
      },

			'homeassignmentdetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'homeassignmentdetaillist button[action=editObject]': {
        click: this.editObject
      },
      'homeassignmentdetaillist button[action=deleteObject]': {
	...
	reloadStore : function(record){
		...
		var store = this.getHomeAssignmentDetailsStore();
		
		store.load({
			params : {
				home_assignment_id : record.get('id')
			}
		});
  ...
  addObject: function(button) {
		...
		var widgetName = 'homeassignmentdetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					home_assignment_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.HomeAssignmentDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('home_assignment_id');
		var list  = this.getList();
