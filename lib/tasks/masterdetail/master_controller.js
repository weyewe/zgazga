Ext.define('AM.controller.Templates', {
  ...
  init: function() {
    this.control({
      'templateProcess templatelist': {
        itemdblclick: this.editObject,
        selectionchange: this.selectionChange,
				afterrender : this.loadObjectList,
      },
      'templateProcess templateform button[action=save]': {
        click: this.updateObject
      },
			'templateProcess templateform customcolorpicker' : {
				'colorSelected' : this.onColorPickerSelect
			},

      'templateProcess templatelist button[action=addObject]': {
        click: this.addObject
      },
      'templateProcess templatelist button[action=editObject]': {
        click: this.editObject
      },
      'templateProcess templatelist button[action=deleteObject]': {
        click: this.deleteObject
			}	,
			
			'templateProcess templatelist button[action=confirmObject]': {
        click: this.confirmObject
      },

			'templateProcess templatelist button[action=unconfirmObject]': {
        click: this.unconfirmObject
      },
			'confirmtemplateform button[action=confirm]' : {
				click : this.executeConfirm
			},
			
			'unconfirmtemplateform button[action=confirm]' : {
				click : this.executeUnconfirm
			},

			'templateProcess templatelist textfield[name=searchField]': {
  ...
  editObject: function() {
	...
	confirmObject: function(){
		...
		var record = this.getList().getSelectedObject();
		if(record){
			var view = Ext.widget('confirmtemplateform');
  ...
  updateObject: function(button) {
	...
	unconfirmObject: function(){
		// console.log("the startObject callback function");
		var view = Ext.widget('unconfirmtemplateform');
		var record = this.getList().getSelectedObject();
		view.setParentData( record );
    view.show();
		// this.reloadRecordView( record, view ) ; 
  ...
  selectionChange: function(selectionModel, selections) {
	...
	updateChildGrid: function(record){
		var templateDetailGrid = this.getTemplateDetailList();
		// templateDetailGrid.setTitle("Purchase Order: " + record.get('code'));
		templateDetailGrid.setObjectTitle( record ) ;
		
		// console.log("record id: " + record.get("id"));
		
		templateDetailGrid.getStore().getProxy().extraParams.template_id =  record.get('id') ;
	...
	reloadRecord: function(record){
		...
		AM.model.Template.load( modifiedId , {
		    scope: list,
