class Api::RollerWarehouseMutationDetailsController < Api::BaseApiController
  ...
  def index
    ...
    @objects = @parent.active_children.joins(:roller_warehouse_mutation, :item).page(params[:page]).per(params[:limit]).order("id DESC")
  ...
  def search
    ...
    if  selected_id.nil?
      @objects = RollerWarehouseMutationDetail.joins(:roller_warehouse_mutation, :item ).where{ 
        ( item.sku  =~ query ) | 
        ( item.name =~ query ) | 
      ...
      @total = RollerWarehouseMutationDetail.joins(:roller_warehouse_mutation, :item ).where{ 
        ( item.sku  =~ query ) | 
        ( item.name =~ query ) | 
        ( item.description  =~ query  )  |
        ( code  =~ query  )  
