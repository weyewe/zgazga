class Api::PaymentVouchersController < Api::BaseApiController
  ...
  def update
    ...
    if params[:confirm].present?  
      if not current_user.has_menu_assignment?( :payment_vouchers, :confirm)
      ...
      begin
        ActiveRecord::Base.transaction do 
          @object.confirm_object(:confirmed_at => params[:payment_voucher][:confirmed_at],
                   :pembulatan =>  params[:payment_voucher][:pembulatan],
                   :status_pembulatan =>  params[:payment_voucher][:status_pembulatan],
          ) 
        end
      ...
      if not current_user.has_menu_assignment?( :payment_vouchers, :unconfirm)
        render :json => {:success => false, :access_denied => "Tidak punya authorisasi"}
        return
      end
