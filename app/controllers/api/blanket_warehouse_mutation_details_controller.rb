class Api::BlanketWarehouseMutationDetailsController < Api::BaseApiController
  ...
  def index
    ...
    @objects = @parent.active_children.joins(:blanket_warehouse_mutation, :item).page(params[:page]).per(params[:limit]).order("id DESC")
  ...
  def search
    ...
    if  selected_id.nil?
      @objects = BlanketWarehouseMutationDetail.joins(:blanket_warehouse_mutation, :item).where{ 
        ( item.sku  =~ query ) | 
        ( item.name =~ query ) | 
      ...
      @total = BlanketWarehouseMutationDetail.joins(:blanket_warehouse_mutation, :item).where{ 
        ( item.sku  =~ query ) | 
        ( item.name =~ query ) | 
        ( item.description  =~ query  )  |
        ( code  =~ query  )  
