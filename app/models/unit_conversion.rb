class UnitConversion < ActiveRecord::Base
  has_many :unit_conversion_details
  ...
  def update_object(params)
    if self.unit_conversion_details.count > 0 
      self.errors.add(:generic_errors,"Sudah memiliki detail")
      return self
    end
    if UnitConversionOrder.where(:unit_conversion_id => self.id).count > 0
