class CompoundUnderlayerUsage < ActiveRecord::Base
    ...
    validate :batch_instance_must_come_from_the_item_specified_in_recovery_order_detail 
    ...
    def valid_recovery_order_detail_id
     ...
     def batch_instance_must_come_from_the_item_specified_in_recovery_order_detail
         ...
         if not recovery_order_detail_object.compound_under_layer_id == batch_instance_object.item_id 
             self.errors.add(:batch_instance_id, "Harus sesuai dengan compound underlayer yang di pilih di Recovery Order Detail")
             return self 
         end
     end
