Ext.define('AM.controller.SalesQuotationDetails', {
  extend: 'Ext.app.Controller',

  stores: ['SalesQuotationDetails', 'SalesQuotations'],
  models: ['SalesQuotationDetail'],

  views: [
    'operation.salesquotationdetail.List',
    'operation.salesquotationdetail.Form',
  ...
  init: function() {
    this.control({
      'salesquotationdetaillist': {
        itemdblclick: this.editObject,
        selectionchange: this.selectionChange ,
				afterrender : this.loadObjectList
      },
      'salesquotationdetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'salesquotationdetaillist button[action=addObject]': {
        click: this.addObject
      },

			'salesquotationdetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'salesquotationdetaillist button[action=editObject]': {
        click: this.editObject
      },
      'salesquotationdetaillist button[action=deleteObject]': {
        click: this.deleteObject
      },
 			'salesquotationdetaillist textfield[name=searchField]': {
	...
	liveSearch : function(grid, newValue, oldValue, options){
		...
		me.getSalesQuotationDetailsStore().getProxy().extraParams.livesearch = newValue; 
	  
		me.getSalesQuotationDetailsStore().load();
	},
	...
	reloadStore : function(record){
		...
		var store = this.getSalesQuotationDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'salesquotationdetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					sales_quotation_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.SalesQuotationDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('sales_quotation_id');
		var list  = this.getList();
