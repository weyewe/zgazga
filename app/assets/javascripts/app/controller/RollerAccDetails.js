Ext.define('AM.controller.RollerAccDetails', {
  extend: 'Ext.app.Controller',

  stores: ['RollerAccDetails', 'RollerAccs'],
  models: ['RollerAccDetail'],

  views: [
    'operation.rolleraccdetail.List',
    'operation.rolleraccdetail.Form',
  ...
  init: function() {
    this.control({
      ...
      'rolleraccdetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'rolleraccdetaillist button[action=addObject]': {
        click: this.addObject
      },

			'rolleraccdetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'rolleraccdetaillist button[action=editObject]': {
        click: this.editObject
      },
      'rolleraccdetaillist button[action=deleteObject]': {
	...
	reloadStore : function(record){
		...
		var store = this.getRollerAccDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'rolleraccdetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					roller_acc_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.RollerAccDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('roller_acc_id');
		var list  = this.getList();
