Ext.define('AM.controller.VirtualOrderClearanceDetails', {
  extend: 'Ext.app.Controller',

  stores: ['VirtualOrderClearanceDetails', 'VirtualOrderClearances'],
  models: ['VirtualOrderClearanceDetail'],

  views: [
    'operation.virtualorderclearancedetail.List',
    'operation.virtualorderclearancedetail.Form',
  ...
  init: function() {
    this.control({
      'virtualorderclearancedetaillist': {
        itemdblclick: this.editObject,
        selectionchange: this.selectionChange ,
				afterrender : this.loadObjectList
      },
      'virtualorderclearancedetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'virtualorderclearancedetaillist button[action=addObject]': {
        click: this.addObject
      },

			'virtualorderclearancedetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'virtualorderclearancedetaillist button[action=editObject]': {
	...
	reloadStore : function(record){
		...
		var store = this.getVirtualOrderClearanceDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'virtualorderclearancedetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					virtual_order_clearance_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.VirtualOrderClearanceDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('virtual_order_clearance_id');
		var list  = this.getList();
    ...
    if (record) {
			record.destroy({
				success : function(record){
					...
					list.getStore().load({
						params : {
							virtual_order_clearance_id : parent_id
						}
					});
				},
				failure : function(record,op ){
