Ext.define('AM.controller.PurchaseRequestDetails', {
  extend: 'Ext.app.Controller',

  stores: ['PurchaseRequestDetails', 'PurchaseRequests'],
  models: ['PurchaseRequestDetail'],

  views: [
    'operation.purchaserequestdetail.List',
    'operation.purchaserequestdetail.Form',
  ...
  init: function() {
    this.control({
      'purchaserequestdetaillist': {
        itemdblclick: this.editObject,
        selectionchange: this.selectionChange ,
				afterrender : this.loadObjectList
      },
      'purchaserequestdetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'purchaserequestdetaillist button[action=addObject]': {
        click: this.addObject
      },

			'purchaserequestdetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'purchaserequestdetaillist button[action=editObject]': {
        click: this.editObject
      },
      'purchaserequestdetaillist button[action=deleteObject]': {
        click: this.deleteObject
      },
 			'purchaserequestdetaillist textfield[name=searchField]': {
	...
	liveSearch : function(grid, newValue, oldValue, options){
		...
		me.getPurchaseRequestDetailsStore().getProxy().extraParams.livesearch = newValue; 
	  
		me.getPurchaseRequestDetailsStore().load();
	},
	...
	reloadStore : function(record){
		...
		var store = this.getPurchaseRequestDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'purchaserequestdetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					purchase_request_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.PurchaseRequestDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('purchase_request_id');
		var list  = this.getList();
