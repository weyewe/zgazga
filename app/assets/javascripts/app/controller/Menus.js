Ext.define('AM.controller.Menus', {
  ...
  init: function() {
    this.control({
      'menuProcess menulist': {
        // itemdblclick: this.editObject,
        selectionchange: this.selectionChange,
				afterrender : this.loadObjectList,
      },
      'menuProcess menuform button[action=save]': {
        click: this.updateObject
      },
			'menuProcess menuform customcolorpicker' : {
				'colorSelected' : this.onColorPickerSelect
			},

      'menuProcess menulist button[action=addObject]': {
        click: this.addObject
      },
      'menuProcess menulist button[action=editObject]': {
        click: this.editObject
      },
      'menuProcess menulist button[action=deleteObject]': {
        click: this.deleteObject
			}	,
			
			'menuProcess menulist button[action=confirmObject]': {
        click: this.confirmObject
      },

			'menuProcess menulist button[action=unconfirmObject]': {
        click: this.unconfirmObject
      },
			'confirmmenuform button[action=confirm]' : {
				click : this.executeConfirm
			},
			
			'unconfirmmenuform button[action=confirm]' : {
				click : this.executeUnconfirm
			},

			'menuProcess menulist textfield[name=searchField]': {
	...
	liveSearch : function(grid, newValue, oldValue, options){
		...
		me.getMenusStore().getProxy().extraParams = {
		    livesearch: newValue
		};
	 
		me.getMenusStore().load();
	},
  ...
  editObject: function() {
	...
	confirmObject: function(){
		...
		var record = this.getList().getSelectedObject();
		if(record){
			var view = Ext.widget('confirmmenuform');
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			var me  = this; 
			var newObject = new AM.model.Menu( values ) ; 
	...
	unconfirmObject: function(){
		// console.log("the startObject callback function");
		var view = Ext.widget('unconfirmmenuform');
		var record = this.getList().getSelectedObject();
		view.setParentData( record );
    view.show();
		// this.reloadRecordView( record, view ) ; 
  ...
  selectionChange: function(selectionModel, selections) {
	...
	updateChildGrid: function(record){
		var templateDetailGrid = this.getMenuDetailList();
		// templateDetailGrid.setTitle("Purchase Order: " + record.get('code'));
		templateDetailGrid.setObjectTitle( record ) ;
		
		// console.log("record id: " + record.get("id"));
		
		templateDetailGrid.getStore().getProxy().extraParams.menu_id =  record.get('id') ;
