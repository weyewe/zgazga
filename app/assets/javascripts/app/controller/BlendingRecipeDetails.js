Ext.define('AM.controller.BlendingRecipeDetails', {
  extend: 'Ext.app.Controller',

  stores: ['BlendingRecipeDetails', 'BlendingRecipes'],
  models: ['BlendingRecipeDetail'],

  views: [
    'master.blendingrecipedetail.List',
    'master.blendingrecipedetail.Form',
  ...
  init: function() {
    this.control({
      'blendingrecipedetaillist': {
        itemdblclick: this.editObject,
        selectionchange: this.selectionChange ,
				afterrender : this.loadObjectList
      },
      'blendingrecipedetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'blendingrecipedetaillist button[action=addObject]': {
        click: this.addObject
      },

			'blendingrecipedetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'blendingrecipedetaillist button[action=editObject]': {
	...
	reloadStore : function(record){
		...
		var store = this.getBlendingRecipeDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'blendingrecipedetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					blending_recipe_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.BlendingRecipeDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('blending_recipe_id');
		var list  = this.getList();
