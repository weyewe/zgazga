Ext.define('AM.controller.WarehouseStockDetails', {
  extend: 'Ext.app.Controller',

  stores: ['WarehouseStockDetails', 'WarehouseStocks'],
  models: ['WarehouseStockDetail'],

  views: [
    'operation.warehousestockdetail.List',
    'operation.warehousestockdetail.Form',
  ...
  init: function() {
    this.control({
      'warehousestockdetaillist': {
        // itemdblclick: this.editObject,
        selectionchange: this.selectionChange ,
				afterrender : this.loadObjectList
      },
      'warehousestockdetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'warehousestockdetaillist button[action=addObject]': {
        click: this.addObject
      },

			'warehousestockdetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'warehousestockdetaillist button[action=editObject]': {
	...
	reloadStore : function(record){
		...
		var store = this.getWarehouseStockDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'warehousestockdetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					warehouse_stock_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.WarehouseStockDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('warehouse_stock_id');
		var list  = this.getList();
