Ext.define('AM.controller.RecoveryWorkProcessDetails', {
  ...
  views: [
    ...
    'operation.recoveryworkprocessdetail.Form',
  ...
  init: function() {
    this.control({
      'recoveryworkprocessdetaillist': {
        itemdblclick: this.editObject,
        selectionchange: this.selectionChange ,
				afterrender : this.loadObjectList
      },
      'recoveryworkprocessdetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'recoveryworkprocessdetaillist button[action=addObject]': {
        click: this.addObject
      },

			'recoveryworkprocessdetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'recoveryworkprocessdetaillist button[action=editObject]': {
	...
	reloadStore : function(record){
		...
		var store = this.getRecoveryWorkProcessDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'recoveryworkprocessdetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					recovery_work_process_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.RecoveryWorkProcessDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('recovery_work_process_id');
		var list  = this.getList();
    ...
    if (record) {
			record.destroy({
				success : function(record){
					...
					list.getStore().load({
						params : {
							recovery_work_process_id : parent_id
						}
					});
				},
				failure : function(record,op ){
