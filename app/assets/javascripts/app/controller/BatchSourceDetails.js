Ext.define('AM.controller.BatchSourceDetails', {
  extend: 'Ext.app.Controller',

  stores: ['BatchSourceDetails', 'BatchSources'],
  models: ['BatchSourceDetail'],

  views: [
    'operation.batchsourcedetail.List',
    'operation.batchsourcedetail.Form',
  ...
  init: function() {
    this.control({
      'batchsourcedetaillist': {
        itemdblclick: this.editObject,
        selectionchange: this.selectionChange ,
				afterrender : this.loadObjectList
      },
      'batchsourcedetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'batchsourcedetaillist button[action=addObject]': {
        click: this.addObject
      },

			'batchsourcedetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'batchsourcedetaillist button[action=editObject]': {
	...
	reloadStore : function(record){
		...
		var store = this.getBatchSourceDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'batchsourcedetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					batch_source_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.BatchSourceDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('batch_source_id');
		var list  = this.getList();
