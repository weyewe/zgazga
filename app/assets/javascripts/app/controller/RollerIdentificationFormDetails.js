Ext.define('AM.controller.RollerIdentificationFormDetails', {
  extend: 'Ext.app.Controller',

  stores: ['RollerIdentificationFormDetails', 'RollerIdentificationForms'],
  models: ['RollerIdentificationFormDetail'],

  views: [
    'operation.rolleridentificationformdetail.List',
    'operation.rolleridentificationformdetail.Form',
  ...
  init: function() {
    this.control({
      'rolleridentificationformdetaillist': {
        itemdblclick: this.editObject,
        selectionchange: this.selectionChange ,
				afterrender : this.loadObjectList
      },
      'rolleridentificationformdetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'rolleridentificationformdetaillist button[action=addObject]': {
        click: this.addObject
      },

			'rolleridentificationformdetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'rolleridentificationformdetaillist button[action=editObject]': {
	...
	reloadStore : function(record){
		...
		var store = this.getRollerIdentificationFormDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'rolleridentificationformdetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					roller_identification_form_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.RollerIdentificationFormDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('roller_identification_form_id');
		var list  = this.getList();
    ...
    if (record) {
			record.destroy({
				success : function(record){
					...
					list.getStore().load({
						params : {
							roller_identification_form_id : parent_id
						}
					});
				},
				failure : function(record,op ){
