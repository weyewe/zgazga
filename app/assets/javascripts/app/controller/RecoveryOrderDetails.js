Ext.define('AM.controller.RecoveryOrderDetails', {
  extend: 'Ext.app.Controller',

  stores: ['RecoveryOrderDetails', 'RecoveryOrders'],
  models: ['RecoveryOrderDetail'],

  views: [
    'operation.recoveryorderdetail.List',
    'operation.recoveryorderdetail.Form',
  ...
  init: function() {
    this.control({
      'recoveryorderdetaillist': {
        itemdblclick: this.editObject,
        selectionchange: this.selectionChange ,
				afterrender : this.loadObjectList
      },
      'recoveryorderdetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'recoveryorderdetaillist button[action=addObject]': {
        click: this.addObject
      },

			'recoveryorderdetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'recoveryorderdetaillist button[action=editObject]': {
	...
	reloadStore : function(record){
		...
		var store = this.getRecoveryOrderDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'recoveryorderdetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					recovery_order_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.RecoveryOrderDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('recovery_order_id');
		var list  = this.getList();
