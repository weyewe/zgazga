Ext.define('AM.controller.ReceivableDetails', {
  extend: 'Ext.app.Controller',

  stores: ['ReceivableDetails', 'Receivables'],
  models: ['ReceivableDetail'],

  views: [
    'operation.receivabledetail.List',
    'operation.receivabledetail.Form',
  ...
  init: function() {
    this.control({
      ...
      'receivabledetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'receivabledetaillist button[action=addObject]': {
        click: this.addObject
      },

			'receivabledetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'receivabledetaillist button[action=editObject]': {
        click: this.editObject
      },
      'receivabledetaillist button[action=deleteObject]': {
	...
	reloadStore : function(record){
		...
		var store = this.getReceivableDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'receivabledetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					receivable_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.ReceivableDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('receivable_id');
		var list  = this.getList();
