Ext.define('AM.controller.PaymentRequestDetails', {
  extend: 'Ext.app.Controller',

  stores: ['PaymentRequestDetails', 'PaymentRequests'],
  models: ['PaymentRequestDetail'],

  views: [
    'operation.paymentrequestdetail.List',
    'operation.paymentrequestdetail.Form',
  ...
  init: function() {
    this.control({
      ...
      'paymentrequestdetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'paymentrequestdetaillist button[action=addObject]': {
        click: this.addObject
      },

			'paymentrequestdetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'paymentrequestdetaillist button[action=editObject]': {
	...
	reloadStore : function(record){
		...
		var store = this.getPaymentRequestDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'paymentrequestdetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					payment_request_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.PaymentRequestDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('payment_request_id');
		var list  = this.getList();
