Ext.define('AM.controller.TransactionDataDetails', {
  extend: 'Ext.app.Controller',

  stores: ['TransactionDataDetails', 'TransactionDatas'],
  models: ['TransactionDataDetail'],

  views: [
    'operation.transactiondatadetail.List',
    'operation.transactiondatadetail.Form',
  ...
  init: function() {
    this.control({
      'transactiondatadetaillist': {
        // itemdblclick: this.editObject,
        selectionchange: this.selectionChange ,
				afterrender : this.loadObjectList
      },
      'transactiondatadetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'transactiondatadetaillist button[action=addObject]': {
        click: this.addObject
      },

			'transactiondatadetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'transactiondatadetaillist button[action=editObject]': {
        click: this.editObject
      },
      'transactiondatadetaillist button[action=deleteObject]': {
        click: this.deleteObject
      },
 			'transactiondatadetaillist textfield[name=searchField]': {
	...
	liveSearch : function(grid, newValue, oldValue, options){
		...
		me.getTransactionDataDetailsStore().getProxy().extraParams.livesearch = newValue; 
	  
		me.getTransactionDataDetailsStore().load();
	},
	...
	reloadStore : function(record){
		...
		var store = this.getTransactionDataDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'transactiondatadetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					transaction_data_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.TransactionDataDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('transaction_data_id');
		var list  = this.getList();
