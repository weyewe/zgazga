Ext.define('AM.controller.PayableDetails', {
  extend: 'Ext.app.Controller',

  stores: ['PayableDetails', 'Payables'],
  models: ['PayableDetail'],

  views: [
    'operation.payabledetail.List',
    'operation.payabledetail.Form',
  ...
  init: function() {
    this.control({
      ...
      'payabledetailform button[action=save]': {
        click: this.updateObject
      },

	 
      'payabledetaillist button[action=addObject]': {
        click: this.addObject
      },

			'payabledetaillist button[action=repeatObject]': {
        click: this.addObject
      },


      'payabledetaillist button[action=editObject]': {
        click: this.editObject
      },
      'payabledetaillist button[action=deleteObject]': {
	...
	reloadStore : function(record){
		...
		var store = this.getPayableDetailsStore();
  ...
  addObject: function(button) {
		...
		var widgetName = 'payabledetailform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					payable_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.PayableDetail( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('payable_id');
		var list  = this.getList();
